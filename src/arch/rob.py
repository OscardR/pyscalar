...
class ROBLine:
    def __init__(
        self, index, n_inst, dest, value=None, ok=False, last=True, flag=DECODED
    ):
        self.index = index
        self.n_inst = n_inst
        self.dest = dest
...
class ReorderBuffer:
    ...
    def insert_line(self, n_inst, dest, value=None, ok=False):
        ...
        if not self.full:
            ...
            for line in self.lines:
                if line is not None and line.dest == dest:
                    line.last = False
            ...
            if None in self.lines:
                while self.lines[self.head] is not None:
                    self.head += 1
                    self.head %= self.size
            else:
    ...
    def get_instruction(self, n_inst):
        for line in self.lines:
            if line is not None and line.n_inst == n_inst:
                return line
        return None
    ...
    def flush_finished(self, how_many=0):
        ...
        while self.lines[self.tail] is not None and self.lines[self.tail].flag == FINISHED:
    ...
    def __str__(self):
        ...
        out += "       [ {:>5} ¦ {:>5} ¦ {:>5} ¦ {:>5} ¦ {:>5} ¦ {:5} ]\n".format(
            "NI", "DEST", "VAL", "OK", "LAST", "FLAG"
        )
        for i, line in enumerate(self.lines):
            out += "[ {:>02} ] [ {:^45} ]{}{}\n".format(
                i,
                line if line is not None else "--- empty ---",
