...
class CPU:
    def __init__(
        self, mem_size=DEF.MEM_SIZE, iw_size=DEF.IW_SIZE, rob_size=DEF.ROB_SIZE, S=DEF.S
    ):
        ...
        self.regs = Registers()
        self.fu = {asm.MULT: FunctionalUnit(asm.MULT), asm.ADD: FunctionalUnit(asm.ADD)}
        self.PC = 0x00
    ...
    def step(self, steps=1):
        try:
            ...
            if self.trap_count >= len(self.stages) - 1:
                ...
                if self.ib.is_empty() and self.rob.is_empty() and self.iw.is_empty():
