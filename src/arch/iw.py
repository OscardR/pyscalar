...
class InstructionWindowLine:
    def __init__(
        self, n_inst, codop=asm.NOP, dest=None, op1=None, ok1=False, op2=None, ok2=False
    ):
        self.n_inst = n_inst
        self.codop = codop
        self.dest = dest
...
class InstructionWindow:
    ...
    def insert_instruction(
        ...
        for pos in range(len(self.lines)):
            if self.lines[pos] == None:
                self.lines[pos] = InstructionWindowLine(
                    n_inst, codop, dest, op1, ok1, op2, ok2
                )
                return True
    ...
    def next_ready_instruction(self):
        ...
        for pos, inst_line in enumerate(self.lines):
            if inst_line != None and (
                (inst_line.ok1 and inst_line.ok2) or inst_line.codop == asm.TRAP
            ):
                return (
                    pos,
                    inst_line.n_inst,
                    Instruction(
                        inst_line.codop, inst_line.dest, inst_line.op1, inst_line.op2
                    ),
                )
    ...
    def get_blocked_instructions(self, rob_index):
        ...
        for inst_line in self.lines:
            if inst_line != None and (
                inst_line.op1 == rob_index
                and not inst_line.ok1
                or inst_line.op2 == rob_index
                and not inst_line.ok2
            ):
