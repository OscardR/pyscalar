        ...
        for _ in range(self.S):
            ...
            if not self.iw.is_full():
                ...
                if codop not in [asm.TRAP, asm.NOP]:
                    ...
                    self.rob.insert_line(n_inst=n_inst, dest=dest, value=None, ok=False)
        ...
        while issued < self.S and possible > 0:
            ...
            # push it to the MULTtiplication functional units
                ...
                if self.fu[asm.MULT].is_empty():
                    ...
                    # Log
                    L.d(
                        "[ I{} ] >> {}".format(n_inst, self.fu[asm.MULT]),
                        "ISS/{}".format(issued),
                    )
                    issued += 1
            ...
            # Same for addition and substraction
                ...
                if self.fu[asm.ADD].is_empty():
                    ...
                    # Log
                    L.d(
                        "[ I{} ] >> {}".format(n_inst, self.fu[asm.ADD]),
                        "ISS/{}".format(issued),
                    )
                    issued += 1
...
class COM(Stage):
    ...
    def execute(self):
        ...
        for line in self.rob.flush_finished():
            ...
            L.d(
                "[ ROB{:>2} ] reg: {} <= {} / OK!".format(
                    line.index, reg.name(line.dest), line.value
                ),
                "COM/REG",
            )
