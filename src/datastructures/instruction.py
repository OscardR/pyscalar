...
class Instruction:
    ...
    def __str__(self):
        op1 = reg.name(self.op1) if self.op1 != None else "\u2014"
        op2 = reg.name(self.op2) if self.op2 != None else "\u2014"
        dest = reg.name(self.dest) if self.dest != None else "\u2014"
        return "{:<4}\t{:<4}\t{:<4}\t{:>4}".format(self.codop, dest, op1, op2)
